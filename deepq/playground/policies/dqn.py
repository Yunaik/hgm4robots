...
class DqnPolicy(Policy, BaseModelMixin):
        ...
        with self.sess.as_default():
            if self.model_type == 'lstm':
                return self.actions_selected_by_q.eval({
                    self.states: [np.zeros(state.shape)] *
                    (self.step_size - 1) + [state]
                })[-1]
            else:
    ...
    def test(self):
        ...
        while not done and (step_counter < self.episode_length):
            ...
            step_counter += 1
            reward += r
            # assert r <= 1.1 and r >= 0, "Error in reward during test"
            ob = new_ob
    ...
    def train(self, config):
        ...
        if self.model_type == 'lstm':
            buffer = ReplayTrajMemory(
                capacity=config.memory_capacity, step_size=self.step_size)
        else:
        ...
        for n_episode in range(config.n_episodes):
            ...
            while not done and (step_counter < config.episode_length):
                ...
                new_ob, r, done, info = self.env.step(a)
                # assert r <= 1.1 and r >= 0, "Reward error"
                step_counter += 1
                ...
                _, q_val, q_target_val, loss, summ_str = self.sess.run(
                    [self.optimizer, self.q, self.q_target,
            ...
            if reward_history and config.log_every_episode \
                ...
                print(
                    "[episodes:{}/step:{}], best:{:.2f}, avg:{:.2f}:{}, lr:{:.4f}, eps:{:.4f}, buff: {}".format(
                        n_episode, step, np.max(reward_history),
                        np.mean(reward_history[-10:]), [round(val, 2)
                                                        for val in reward_history[-5:]],
                        lr, eps, buffer.size
                    ))
            ...
            if n_episode % 100 == 0 and n_episode != 0:
                ...
                if test_reward > best_reward:
                    ...
                    self.save_checkpoint(step=n_episode, is_best=True)
